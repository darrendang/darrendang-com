...
The films are intentionally silent: no background music, narration, or audio track.
"""
...
def encode(slides, out):
    ...
    cmd=["ffmpeg","-y",*inputs,"-filter_complex",";".join(filters),"-map","[vout]","-an","-t","30","-r",str(FPS),"-c:v","libx264","-preset","medium","-crf","29","-pix_fmt","yuv420p","-movflags","+faststart",out]
    subprocess.run(cmd,check=True)

def build(slidespec,name):
    ...
    out=os.path.join(OUT,name+'.mp4'); encode(slidepaths,out)
    return out,poster
...
def main():
    ...
    jobs=[
        (book1,"for-those-who-come-after-us-book-film"),
        (book2,"wisdom-has-no-rank-book-film"),
        (book3,"dots-book-film"),
    ]
    for job in jobs:
